  ...
  nMcoverage()//1, _model.read_length)//zeros(model.read_length,1);
{
  ...
  unsigned dim_size3[3]={_model.number_D_gene, (unsigned)(_model.max_D_cut-_model.min_D_cut+1), (unsigned)(_model.max_D_cut-_model.min_D_cut+1) };
  nPcutDlcutDr_given_D.initialize(3, dim_size3, 0.0);
  
  unsigned dim_size[1]={_model.number_V_gene};
  nPV.initialize(1, dim_size, 0.0);
  dim_size2[0]=maxGeneIndex_D;
  dim_size2[1]=maxGeneIndex_J;
