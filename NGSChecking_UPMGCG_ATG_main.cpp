...
static string reverseFile_name("primers_Nested_UPM_Reverse_ATG");//input ifle for reverse UPM
...
int main(int argc, char* argv[])
{
  ...
  const char *opts = "hvr:g:e:m:s:k:n:l:";
  //a: the input file name adaptor
  //b: the input file name barcode
  //**deleted***f: the input file name forward primer
  ...
  //***deleted***p: offset in for the forward
  //***deleted***q: offset for the reverse end
  ...
  string outputFileReverse_name=(sequenceFile_name+".mapReverse_checkUPMGCG.fasta");//map reverse
  ...
  cout<<"\tThe output file name : \""
    //<<outputFileBoth_name<<"\",\""<< outputFileForward_name<<"\",\""
      << outputFileReverse_name<<"\",\""
    //<<outputFileNone_name<<"\";\n"
...
//now we have everything, we just need to do the job, I mean mapping, here.
  CheckingUPMGCGvsATC(/*vec_forward_seq,*/ vec_reverse_seq, vec_seq, 
		  sm, gapopen, gapextension,
		   MismatchRateThreshold, MinimumOverlapLength, /*OffsetForward, OffsetReverse, 
								  outputFileBoth_name, outputFileForward_name, */
		      outputFileReverse_name,outputFileNone_name/*,
...
static void printUsage(int argc, char* argv[])
{
  ...
  */
  ...
  cout<<"\t\t-e gapextension -- the gap extension value, will be turned into negative\n"
      <<"\t\t\t if it is not negative already\n";
