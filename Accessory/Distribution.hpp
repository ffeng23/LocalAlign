...
//this is the template for all the probability distribution, the function is following
//R style.
//check below
...
/*****NOTE******
 * for now, we don't use it. we need to think more about the generic interface
 * because for now it is not handling well the continuous vs. discrete cases
 *
 * will do more later to make thing decent
 */
...
class Distribution
{
  ...
  //set parameter
  //this is very generic functions to set the parameters for the specific distribution
  //the parameter could be double or intergar, and could be any number of iterms
  virtual void SetParams(const double* _pd, const unsigned _size_pd, const singed long int* _pi, const unsigned _size_pi)=0;
  
  //this is the dnorm, density/probability
  virtual double Density(const double& _x)=0;

  virtual double Density(const signed long& _x)=0;
  //this is the pnorm, cumulative probability. prob(x<_x), pnorm ,etc
  virtual double Probability(const double& _x)=0;
  virtual double Probability(const signed long& _x)=0;
  //this is the qnorm,
  virtual double Quantile(const double& _x)=0;
